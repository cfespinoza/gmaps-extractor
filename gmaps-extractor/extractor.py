    ...
    try:
        for i in range(1, 11):
            ...
            while not processed_page:
                without_comments = ["comments" not in processed_rest[rest].keys() for rest in processed_rest].count(
                    True)
                logger.info(
                    "Trying to extract comments for -{without_comments}- of total restaurants: -{total}-".format(
                ...
                for r in aux_results:
                    ...
                    if processed_rest.get(name):
                        if processed_rest.get(name).get("comments") is None:
                            ...
                            openning_obj = get_info_obj(driver,
                                                        "//*[@id='pane']/div/div[1]/div/div/div[@jsaction='pane.info.dropdown;keydown:pane.info.dropdown;focus:pane.focusTooltip;blur:pane.blurTooltip;']/div[3]")
                            processed_rest[name]["opennig_hours"] = openning_obj.get_attribute("aria-label").split(
                            ...
                            logger.info(
                                "Comments has been processed for restaurant -{restaurant}-".format(restaurant=name))
                    else:
                        logger.info(
                            "There are a restaurant that has not been preprocessed -{restaurant}-".format(
                                restaurant=name))
                        processed_rest[name] = {
                            "comments": []
                        }
                ...
                processed_page = all(total_processed) or len(processed_rest_list) >= len(aux_results) or len(
                    aux_results) == 0
        ...
        logger.info(
            "The scraping has finished and there have been {total_rest} found".format(total_rest=len(processed_rest)))
